# Acertpix API OCR MCP Server

Servidor MCP para integração com a API OCR da Acertpix.
...
-   **Consulta de OCR de documento por chave:** Permite consultar o OCR de documento de uma pessoa física.
...
3. Para usar Docker, build a imagem
    ...
    ```bash
    docker build -t acertpix-api-orc .
    ```
...
```json
{
    "servers": {
        "acertpix-api-ocr-docker": {
            ...
            "--rm", 
            "-p", "8000:8000", "acertpix-api-ocr"]
        },
        "acertpix-api-ocr-src": {
            "command": "python",
            "args": [
                "-m",
                "acertpix_api_ocr"
...
resultado = await server.call_tool("consultar-ocr", {
    "chave": "12345678900"
})
```
